// ============================================================
//  ExpaHome — Contact Form Handler (Vercel Serverless Function)
//  Uses Resend API to send inquiry + auto-reply emails
// ============================================================
//
//  DEPLOYMENT:
//  1. Place this file at /api/contact.js in your project root
//  2. Add RESEND_API_KEY to Vercel Environment Variables
//  3. Deploy — your form will POST to /api/contact
//
//  For Netlify: rename folder from /api to /netlify/functions
//  and change export syntax (see netlify-version.js)
// ============================================================

export default async function handler(req, res) {
  // === CORS headers (allow your domain) ===
  res.setHeader('Access-Control-Allow-Origin', 'https://expahome.bg');
  ...
  try {
    const { name, email, phone, model, message, honeypot } = req.body;
    ...
    // ============================================================
    const inquiryHtml = buildInquiryEmail({
      name: safeName,
      email: safeEmail,
      phone: safePhone,
      model: safeModel,
    ...
    if (!inquiryResponse.ok) {
      ...
      console.error('Resend inquiry error:', errorData);
    ...
    if (!autoReplyResponse.ok) {
      ...
      // Don't fail the whole request — inquiry was already sent successfully
    }
...
// ============================================================
//  EMAIL TEMPLATE: Inquiry notification
// ============================================================
      ...
      <tr>
        <td style="padding:20px 40px 30px 40px;">
          ...
          <div style="background-color:#faf7f2;border-left:3px solid #d4a574;padding:20px 24px;border-radius:2px;">
            <p style="margin:0;color:#3d2e1f;font-family:'Georgia',serif;font-size:15px;line-height:1.7;font-style:italic;">"${message}"</p>
          </div>
        </td>
      </tr>
      <tr>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#faf7f2;border-radius:4px;"><tr><td style="padding:24px 28px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              ...
              <tr><td>
                ...
                <span style="color:#4a3d30;font-family:'Georgia',serif;font-size:14px;font-style:italic;line-height:1.6;">"${message}"</span>
              </td></tr>
            </table>
          </td></tr></table>
        </td>
                ...
                return `<tr><td style="padding:12px 0;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
                    <td width="50" style="vertical-align:top;">
                      <div style="width:40px;height:40px;background-color:#2c1810;border-radius:50%;text-align:center;line-height:40px;color:#d4a574;font-family:'Georgia',serif;font-size:16px;font-weight:600;">${i + 1}</div>
                    </td>
